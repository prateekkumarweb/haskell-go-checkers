...
render game@(Game (BoardMap board) _ _ (Square x y) ) = pictures [boardBox, (pictures blackboxes), blueSquare, (pictures pieces), (pictures kings)]
    where blackboxes = [translate (fromIntegral (2*x -9)*40) (fromIntegral (9-2*y)*40) $ color (greyN 0.25) $ rectangleSolid 80 80 | (Square x y,b) <- assocs board ]
          ...
          kings = [translate (fromIntegral (2*x -9)*40) (fromIntegral (9-2*y)*40) $ scale 0.2 0.2 $ text "K" | (Square x y, Piece pl pt) <- assocs board, pt == King]
